...
int main() {
    ...
    double maxMegaHeights = 0., sumMegaHeights = 0., sumTimeElapsed = 0., time0, time1;
    float volume;
    int numTiles;

    printf("Using %d threads\n"
                     "There are %d height tiles to calculate.\n"
                     "Running same instruction %d times\n", NUMT, NUMNODES*NUMNODES, NUMTRIES );
    ...
    for (int i = 0; i < NUMTRIES; i++) {
        ...
        for (int i = 0; i < numTiles; i++) {
            ...
            volume += Height(iu, iv) * type / numTiles;
        }

        time1 = omp_get_wtime();

        // collect stats
        printf("Run #%d, Volume: %lf\n", i, volume);
        double megaHeights = (double)numTiles/(time1-time0)/1000000.;
        sumMegaHeights += megaHeights;
        if (megaHeights > maxMegaHeights)
            maxMegaHeights = megaHeights;
        sumTimeElapsed += (time1-time0);
    }

    double avgMegaHeights = sumMegaHeights/(double)NUMTRIES;
    double avgTimeElapsed = sumTimeElapsed/(double)NUMTRIES;
