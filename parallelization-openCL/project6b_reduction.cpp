...
//        // fill in with 1s for testing to see if reduction worked
//        for (int i = 0; i < NUM_ELEMENTS; i++) {
//            hA[i] = hB[i] = 1.5;
//        }
        ...
        status = clSetKernelArg(kernel, 2, sizeof(float), NULL); //local prods arrya - 1 per work-item
        if (status != CL_SUCCESS)
...
//    }
        ...
        //global work size;local work size;number of work groups;gigaMultsPerSecond
        double gigaMultPerSecond = (double) NUM_ELEMENTS / (time1 - time0) / 1000000000.;

        if (gigaMultPerSecond > maxGigaMultPerSecond)
            maxGigaMultPerSecond = gigaMultPerSecond;
    ...
    printf("%10d;    %8d;    %4d;    %10.3lf\n",
           NUM_WORK_GROUPS, NUM_ELEMENTS, LOCAL_SIZE, maxGigaMultPerSecond);
